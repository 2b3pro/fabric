...
func (c *Client) Send(ctx context.Context, msgs []*chat.ChatCompletionMessage, opts *domain.ChatOptions) (string, error) {
	...
	// Check if schema content is provided and defines a "citations" field
	schemaDefinesCitations := false
	if opts.SchemaContent != "" {
		var schema map[string]interface{}
		if err := json.Unmarshal([]byte(opts.SchemaContent), &schema); err == nil {
			if properties, ok := schema["properties"].(map[string]interface{}); ok {
				if _, ok := properties["citations"]; ok {
					schemaDefinesCitations = true
				}
			}
		}
	}

	// Handle structured output with citations embedded in JSON if schema defines it
	if opts.SchemaContent != "" && schemaDefinesCitations && len(citations) > 0 {
	...
	} else if len(citations) > 0 { // Original logic for non-structured output or if schema doesn't define citations
		content += "\n\n# CITATIONS\n\n"
		for i, citation := range citations {
			content += fmt.Sprintf("- [%d] %s %s\n", i+1, citation.Title, citation.URL)
		}
...
func (c *Client) SendStream(msgs []*chat.ChatCompletionMessage, opts *domain.ChatOptions, channel chan string) error {
	...
	go func() {
		...
		if lastResponse != nil {
			...
			if len(citations) > 0 {
				if opts.SchemaContent != "" && schemaDefinesCitations {
					...
					// Non-structured output or schema doesn't define citations: Append citations as plain text
					channel <- "\n\n# CITATIONS\n\n"
					for i, citation := range citations {
						channel <- fmt.Sprintf("- [%d] %s %s\n", i+1, citation.Title, citation.URL)
					}
